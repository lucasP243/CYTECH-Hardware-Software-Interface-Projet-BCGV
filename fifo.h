 ...
 * \brief       This files allow a user to create and manage a fifo buffer
 * (circular buffer). \details     This file gives the interfaces to be able to
 * create and manage the fifo buffer. Concerning threads, the fifo is designed
...
#define FIFO_LOST                                                              \
  (3) /* (read) 'sz_or_count' set to the count of lost items                   \
       */
...
/**
 * \brief   Internal structure of the global FIFO header, used internally by
 * FIFO implementation \details Contains read and write index, but client does
 * not need to know its content
 *
 */
typedef struct hsi_fifo_s
    hsi_fifo_t; /* forward declaration for the opaque structure */
 ...
 *                            NULL in case of error: the size is too small or
 * NULL pointer given
 */
 ...
 *              if there was rejected data on push, return special id
 * (FIFO_LOST) \param       p_fifo      : Pointer to the fifo object \param[out]
 * item        : Pointer on item data to set \return      int32_t : FIFO_DATA if
 * a data is returned FIFO_EMPTY if fifo is empty FIFO_LOST if there was
 * rejected data on push FIFO_FAILURE if a given pointer is NULL
 */
int32_t fifo_read(hsi_fifo_t *p_fifo, fifo_item_t *item);

/**
 * \brief       Place the read index to the next item, and call fifo_read with
 * this new index \details \param       p_fifo      : Pointer to the fifo object
