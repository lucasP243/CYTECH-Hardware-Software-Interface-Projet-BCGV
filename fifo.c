...
struct hsi_fifo_s {
  ...
  volatile uint32_t rejected_count; /*!< count the number of rejected values in
                                       push (if fifo is full for example) */
 ...
 *              Warning : We must to be sure that *src is not set during
 * execution of this function \param      	src     : pointer to variable to
 * set \param       value   : value to set \return   	int32_t : 0 if
 * everything is OK -errno otherwise
 */
...
hsi_fifo_t *fifo_get_pointer(void) { return &fifo; }
...
int32_t fifo_read(hsi_fifo_t *p_fifo, fifo_item_t *item) {
  ...
  if (p_fifo == NULL || item == NULL) {
    ...
    p_fifo->rejected_count &= 0x0;
    ret = FIFO_LOST;
  } else if (p_fifo->read_index ==
             p_fifo->write_index) { /* read = write -> empty fifo */
    ret = FIFO_EMPTY;
  } else {
